...
DEBUG_FILE     = "C:/Users/Georg/Documents/nanabanana_debug.txt"
...
def tensor_to_bytes(tensor, max_size=1024):
    ...
    if max(w, h) > max_size:
        ...
        print("[NanaBanana by George] Imagen reducida a {}x{}".format(img.width, img.height))
    buf = BytesIO()
...
def poll_prediction(prediction_id, api_key, timeout=300):
    ...
    while elapsed < timeout:
        ...
        if status == "succeeded":
            ...
            log_debug("[NanaBanana by George] FALLO — error: {} logs: {}".format(error, logs[-1000:]))
            raise RuntimeError("[NanaBanana by George] {} — error: {} | logs: {}".format(
                status, error, logs[-500:]))
...
def build_payload_nanobanana(prompt, aspect_ratio, resolution, output_format,
    ...
    """Payload para google/nano-banana-2"""
    return {
        "prompt":        prompt,
...
def build_payload_gemini(prompt, system_prompt, thinking_level, response_modalities,
                         seed, image_urls):
    """Payload para google/gemini-3-pro"""
    payload = {
        "prompt":              prompt,
...
class NanaBananaByGeorge:
    ...
    def generate(self, api_key, prompt, model, seed, control_despues_de_generar,
        ...
        # Subir imágenes si vienen conectadas
        image_urls = []
        ...
        log_debug("[NanaBanana by George] Payload: {}".format(json.dumps(payload_input, default=str)[:1000]))

        resp = requests.post(url, json={"input": payload_input}, headers=headers, timeout=30)
        log_debug("[NanaBanana by George] Prediction response: {} {}".format(resp.status_code, resp.text[:300]))
        resp.raise_for_status()
...
NODE_CLASS_MAPPINGS = {
    "NanaBananaByGeorge": NanaBananaByGeorge,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "NanaBananaByGeorge": "NanaBanana by George",
}
